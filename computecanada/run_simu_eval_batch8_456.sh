...
do
  ...
  do
    ...
    do 
      for sz in 100 500
      do 
        sbatch --job-name=$sz.$se.$mt ./computecanada/simu_eval_SAMseq_test.sh $se $sz $mt $nm $workdir
      done
    done
  done
done
...
do
  ...
  do
    ...
    do
      for sz in 100 500
      do
        sbatch --job-name=$sz.$se.$mt ./computecanada/simu_eval_DEsingle_test.sh $se $sz $mt $nm $workdir
      done
    done
  done
done
